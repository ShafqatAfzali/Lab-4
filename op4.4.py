...
N_vektor = [100, 1000, 10000, 20000, 30000, 40000]
...
def plott_std(N_vekt):
    ...
    for i in N_vekt:
        ...
        teoretisk_std = 4*np.sqrt((np.pi/4)*(1-(np.pi/4))/i)
        plt.axhline(teoretisk_std, c="lightblue")

    xx = np.log10(N_vekt)
    plt.scatter(xx, std_av_vekt, color="black")
    plt.errorbar(xx, std_av_vekt, yerr=u_std_vekt, color="blue")
    plt.title("std vs N")
    plt.xlabel("log(N)")
    plt.ylabel("std")
    plt.legend(["teoretisd std=lysblå", "std verdier=blå"],
               loc="upper right")
    plt.show()
